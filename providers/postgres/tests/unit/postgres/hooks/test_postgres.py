...
class TestPostgresHookConnPPG3:
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_raises_not_implemented_without_psycopg3(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_aget_conn(self, mocker):
        ...
        result = await self.db_hook.aget_conn()
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_with_cursor(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_with_invalid_cursor(self, mocker):
        self.connection.extra = '{"cursor": "mycursor"}'
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_adds_notice_handler_when_enabled(self, mocker):
        mock_async_connect = mocker.patch("psycopg.AsyncConnection.connect", new_callable=mock.AsyncMock)
        mocker.patch("airflow.providers.postgres.hooks.postgres.register_default_adapters")
        mock_connection = mocker.MagicMock()
        mock_async_connect.return_value = mock_connection
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_no_notice_handler_when_disabled(self, mocker):
        mock_async_connect = mocker.patch("psycopg.AsyncConnection.connect", new_callable=mock.AsyncMock)
        mocker.patch("airflow.providers.postgres.hooks.postgres.register_default_adapters")
        mock_connection = mocker.MagicMock()
        mock_async_connect.return_value = mock_connection
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_with_namedtuple_cursor(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_with_realdictcursor_raises(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_with_extra(self, mocker):
        self.connection.extra = '{"connect_timeout": 3}'
        mock_async_connect = mocker.patch("psycopg.AsyncConnection.connect", new_callable=mock.AsyncMock)
        mocker.patch("airflow.providers.postgres.hooks.postgres.register_default_adapters")
    ...
    @pytest.mark.asyncio
    async def test_aget_conn_ignores_unknown_extras(self, mocker):
        self.connection.extra = '{"connect_timeout": 3, "evil_callback": "os.system", "sslmode": "require"}'
        mock_async_connect = mocker.patch("psycopg.AsyncConnection.connect", new_callable=mock.AsyncMock)
        mocker.patch("airflow.providers.postgres.hooks.postgres.register_default_adapters")

        await self.db_hook.aget_conn()

        kwargs = mock_async_connect.await_args.kwargs
        assert kwargs["connect_timeout"] == 3
        assert kwargs["sslmode"] == "require"
        assert "evil_callback" not in kwargs

    @pytest.mark.asyncio
    async def test_aget_conn_with_options(self, mocker):
        mock_async_connect = mocker.patch("psycopg.AsyncConnection.connect", new_callable=mock.AsyncMock)
        mocker.patch("airflow.providers.postgres.hooks.postgres.register_default_adapters")
