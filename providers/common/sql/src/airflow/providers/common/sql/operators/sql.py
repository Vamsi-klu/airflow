...
class SQLExecuteQueryOperator(BaseSQLOperator):
    ...
    :param enforce_read_only: (optional) Only effective when ``deferrable=True``. If the triggerer restarts
        ...
        **only** if your statements are idempotent. This flag does not make writes safe if they are not
        idempotent. (default: True). Result rows are serialized through a ``TriggerEvent`` and rejected
        above 1 MiB; use ``deferrable=False`` or unload large results.
    ...
    def _require_deferrable_hook(self) -> DbApiHook:
        hook = self.get_db_hook()
        if not hook.supports_async_execution():
            raise ValueError(
                f"{hook.__class__.__name__} does not provide a native async execution path. "
                "SQLExecuteQueryOperator deferrable mode only runs on hooks that override aget_conn "
                "with a non-blocking driver. Set deferrable=False or use PostgresHook."
            )
        if self.enforce_read_only and not hook.supports_readonly_execution():
            raise ValueError(
                f"{hook.__class__.__name__} cannot enforce read-only execution. "
                "Set enforce_read_only=False for idempotent SQL, or deferrable=False."
            )
        return hook
    ...
    def execute_complete(self, context: Context, event: dict[str, Any] | None = None) -> Any:
        ...
        results = SQLExecuteQueryTrigger.deserialize_sql_value(event.get("results"))
        ...
        if return_single_query_results(self.sql, self.return_last, self.split_statements):
            ...
            # A bare string is treated as one scalar cell so the replay cursor still iterates rows.
