...
from airflow.providers.common.sql.triggers.sql import MAX_TRIGGER_RESULT_BYTES, SQLExecuteQueryTrigger
...
class TestSQLExecuteQueryTrigger:
    ...
    def test_constructor_keeps_legacy_positional_args(self):
        trigger = SQLExecuteQueryTrigger("SELECT 1", "test_conn", {"foo": "bar"})
        assert trigger.hook_params == {"foo": "bar"}
        assert trigger.autocommit is False
        assert trigger.split_statements is None
        assert trigger.return_last is True
        assert trigger.fetch_results is True
        assert trigger.database is None
    ...
    def test_aget_hook_applies_database_override(self):
        mock_hook = self._make_mock_hook()
        mock_hook.conn_type = "postgres"
        trigger = self._make_trigger(database="analytics")
        with mock.patch(
            "airflow.providers.common.sql.triggers.sql.get_async_hook",
            new=mock.AsyncMock(return_value=mock_hook),
        ):
        ...
        assert mock_hook.database == "analytics"

    def test_jsonsafe_value_round_trips_postgres_types(self):
        raw = [
            (
                Decimal("12.50"),
    ...
    def test_check_result_bound_rejects_oversized_payload(self):
        oversized = "x" * (MAX_TRIGGER_RESULT_BYTES + 1)
        with pytest.raises(ValueError, match="TriggerEvent limit"):
            SQLExecuteQueryTrigger.check_result_bound(oversized, limit=MAX_TRIGGER_RESULT_BYTES)

    def test_run_rejects_oversized_results(self):
        mock_hook = self._make_mock_hook()
        mock_hook.arun.return_value = [("x" * (MAX_TRIGGER_RESULT_BYTES + 1),)]
        ...
        assert events[0].payload["status"] == "error"
        assert "TriggerEvent limit" in events[0].payload["message"]
