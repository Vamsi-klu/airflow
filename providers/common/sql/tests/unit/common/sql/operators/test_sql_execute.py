...
class TestSQLExecuteQueryOperatorDeferrable:
    def _patch_hook(self, op, *, async_ok=True, readonly_ok=True):
        hook = MagicMock(spec=DbApiHook)
        hook.supports_async_execution.return_value = async_ok
        hook.supports_readonly_execution.return_value = readonly_ok
        op.get_db_hook = MagicMock(return_value=hook)
        return hook
    ...
    def test_execute_fails_on_worker_when_hook_lacks_async(self):
        op = SQLExecuteQueryOperator(task_id=TASK_ID, sql="SELECT 1", deferrable=True)
        self._patch_hook(op, async_ok=False)
        with pytest.raises(ValueError, match="native async execution path"):
            op.execute({})

    def test_execute_fails_on_worker_when_hook_lacks_read_only(self):
        op = SQLExecuteQueryOperator(task_id=TASK_ID, sql="SELECT 1", deferrable=True, enforce_read_only=True)
        self._patch_hook(op, readonly_ok=False)
        with pytest.raises(ValueError, match="cannot enforce read-only execution"):
            op.execute({})

    def test_execute_passes_hook_params_and_database_to_trigger(self):
        op = SQLExecuteQueryOperator(
            task_id=TASK_ID,
            sql="SELECT 1",
            deferrable=True,
            conn_id="sql_default",
            database="analytics",
            hook_params={"options": "-c statement_timeout=1s"},
        )
        self._patch_hook(op)
        with pytest.raises(TaskDeferred) as exc:
            op.execute({})
        trigger = exc.value.trigger
        assert trigger.hook_params == {"options": "-c statement_timeout=1s"}
        assert trigger.database == "analytics"
        assert trigger.conn_id == "sql_default"

    def test_execute_complete_round_trips_serialized_values(self):
        from decimal import Decimal

        encoded = SQLExecuteQueryTrigger._jsonsafe_results([(Decimal("1.25"),)])
        op = SQLExecuteQueryOperator(task_id=TASK_ID, sql="SELECT 1", deferrable=True, do_xcom_push=True)
        result = op.execute_complete(
            context={},
            event={"status": "success", "results": encoded, "descriptions": [[("amount",)]]},
        )
