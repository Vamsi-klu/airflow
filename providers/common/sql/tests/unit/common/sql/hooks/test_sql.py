...
class TestDbApiHook:
    ...
    @pytest.mark.db_test
    def test_supports_readonly_execution_false_by_default(self):
        hook = mock_db_hook(DbApiHook)
        assert hook.supports_readonly_execution() is False

    @pytest.mark.db_test
    def test_supports_readonly_execution_true_when_overridden(self):
        class ReadOnlyDbApiHook(DbApiHook):
            async def _aenter_read_only(self, conn):
                return None

        hook = mock_db_hook(ReadOnlyDbApiHook)
        assert hook.supports_readonly_execution() is True
